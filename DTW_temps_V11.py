...
# Cette valeur dépend de la granularité de votre matrice d'alignement et de la taille des données
...
# Calculer les points hors zone d'acceptabilité en utilisant la matrice d'alignement
points_hors_zone = []
for i, (indice_em, indice_lu) in enumerate(zip(indices_seq1, indices_seq2)):
    # Vérifier si le point est en dehors de la zone d'acceptabilité
    if not (indice_em - marge <= indice_lu <= indice_em + marge):
        # Ajouter l'indice de temps de data_em à la liste
        points_hors_zone.append(indice_em)

# Assurer l'unicité et le tri des indices pour éviter les doublons
points_hors_zone = sorted(set(points_hors_zone))

# Initialiser une liste pour stocker les groupes de points consécutifs hors zone
groupes_hors_zone = []
groupe_actuel = []

# Initialiser une variable pour suivre l'état actuel ("trop rapide" ou "trop lent")
etat_actuel = None

for i, (indice_em, indice_lu) in enumerate(zip(indices_seq1, indices_seq2)):
    if not (indice_em - marge <= indice_lu <= indice_em + marge):
        etat_point = "Trop rapide" if indice_em < indice_lu else "Trop lent"
        if etat_point != etat_actuel:
            # Si l'état change, sauvegarder le groupe actuel et en commencer un nouveau
            if groupe_actuel:
                groupes_hors_zone.append((etat_actuel, groupe_actuel))
            groupe_actuel = [i]
            etat_actuel = etat_point
        else:
            groupe_actuel.append(i)
    elif groupe_actuel:
        # Si nous sortons de la zone hors acceptabilité, finaliser le groupe actuel
        groupes_hors_zone.append((etat_actuel, groupe_actuel))
        groupe_actuel = []
        etat_actuel = None
...
# Dessiner le graphe et annoter les zones
plt.figure(figsize=(8, 6))
plt.plot(data_em[:, 1], data_em[:, 2], label='Votre mouvement', linestyle='-')

for etat, groupe in groupes_hors_zone:
    # Trouver les temps de début et de fin du groupe
    temps_debut = data_em[indices_seq1[groupe[0]], 1]
    temps_fin = data_em[indices_seq1[groupe[-1]], 1]
    
    # Marquer la zone
    plt.axvspan(temps_debut, temps_fin, color='lightpink', alpha=0.5)
    
    # Placer une annotation au milieu du groupe
